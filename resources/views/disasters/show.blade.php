...
@section('content')
<div class="min-h-screen bg-white text-slate-100">
    <div class="max-w-6xl mx-auto py-10 px-4 sm:px-6 lg:px-8 space-y-8">

        {{-- Breadcrumb / Back --}}
        <div class="flex items-center justify-between gap-4">
            <a href="{{ route('disasters.index') }}"
               class="inline-flex items-center text-sm text-black hover:text-black">
        ...
        {{-- ==== SEMUA KONTEN DALAM 1 KOTAK BESAR ==== --}}
        <div class="bg-slate-900/80 border border-slate-800 rounded-2xl p-6 space-y-8">
                ...
                <div class="flex-1 space-y-4">
                    <div class="flex items-start justify-between gap-4">
                        <div>
                            ...
                            {{-- Kalau mau tampilkan lokasi, buka komentar ini --}}
                            {{-- <p class="mt-1 text-sm text-slate-400">
                                Lokasi: {{ $disaster->location ?? '-' }}
                            </p> --}}
                        </div>

                        {{-- Status (hanya admin & partner) --}}
                        @auth
                            @if(auth()->user()->isAdmin() || auth()->user()->isPartner())
                                <span class="inline-flex items-center rounded-full px-3 py-1 text-xs font-semibold
                                    @if($disaster->status === 'active')
                                        bg-emerald-500/10 text-emerald-300 border border-emerald-500/40
                                    @elseif($disaster->status === 'draft')
                                        bg-yellow-500/10 text-yellow-300 border border-yellow-500/40
                                    @else
                                        bg-slate-600/10 text-slate-300 border border-slate-500/40
                                    @endif">
            ...
            {{-- Konten bawah: kiri (deskripsi + tautan) & kanan (kalkulator) --}}
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start">

                {{-- Deskripsi + tautan sosial (kiri) --}}
                <div class="lg:col-span-2 space-y-6">
                    ...
                    {{-- Tautan Sosial --}}
                    <div>
                        <h2 class="text-lg font-semibold mb-3">Tautan Sosial</h2>
                        ...
                        <ul class="space-y-2 text-sm">
                            ...
                            @if ($disaster->links->count())
                                @foreach ($disaster->links as $l)
                                    <li>
                                        <span class="uppercase text-xs text-slate-400">{{ $l->platform }}</span> :
                                        <a class="text-emerald-400 hover:text-emerald-300 break-all"
                            ...
                            @if(
                                !($disaster->instagram_url ?? false)
                                && !$disaster->links->count()
                            )
                ...
                {{-- Kalkulator Kontribusi (kanan) --}}
                <div
                    id="contribution-calculator"
                    class="bg-slate-800/70 border border-slate-700 rounded-2xl p-6 space-y-4"
                    x-data="{
                        amount: '',
                ...
                >
                    ...
                    <div class="space-y-2">
                        ...
                        <input
                            ...
                            "
                            class="w-full rounded-lg bg-slate-900 border border-slate-700 px-4 py-2 text-slate-100
...
@vite('resources/js/donation-calculator.js')
@endsection
